    ...
    <div className="h-auto mb-5">
      ...
      <div className="tab w-[90%] max-w-4xl mx-auto mt-5 text-center">
            ...
            {currentCategory.skills.map((obj,index) => (
              <div key={index} className="w-full  px-3 py-2 ">
                ...
                <div className="w-full rounded-full h-3 bg-gray-300 mt-1 ">
                  <div className={`h-full  rounded-full transition-all duration-500 ${getColorByLevel(obj.level)}`} style={{ width: `${obj.level}%` }}>
        ...
        <div className="mt-20 rounded-md p-4 " >
          <h3 className="text-3xl font-bold text-center mb-8">Technologies I Work With</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-8">
            {AllTechnologies.map((tech) => (
              <div key={tech} className="flex flex-col items-center">
                <div className="w-16 h-16 bg-purple-600 rounded-full flex items-center justify-center mb-2">
                  <span className="text-xl text-white font-bold font-mono">{tech.charAt(0)}</span>
                </div>
                <span className="text-sm font-medium font-mono">{tech}</span>
              </div>
            ))}
          </div>
        </div>
